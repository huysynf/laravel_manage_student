        ...
        <form method="post" id="update-role-form" action="{{route('roles.update',$role->id)}}"
            ...
            {{method_field('put')}}
                <div class="form-group">
                    ...
                    <input type="text" class="form-control role-name" name="name" value="{{old('name')==""?$role->name:old('name')}}"
                           required>
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control role-slug" name="slug" value="{{old('slug')==""?$role->slug:old('slug')}}"
                           required>
                    @error('slug')
                    <span class="text-danger">{{ $message }}</span>
                    @enderror
                                ...
                                <label class="form-check-label pl-4">
                                    @if($permission->slug=="not-permission")
                                        <input type="checkbox" class="form-check-input  un-select-all"
                                               name="permissions[]"
                                               value="{{$permission->id}}" {{( $listPermission->contains($permission->id)) ? 'checked=checked' : '' }}>{{$permission->name}}
                                    @else
                                        <input type="checkbox" class="form-check-input   select-item"
                                               name="permissions[]"
                                               {{( $listPermission->contains($permission->id)) ? 'checked=checked' : '' }}
